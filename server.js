...
// Async function to load Firebase service account key
async function loadServiceAccountKey() {
  try {
    const data = await fs.readFile(path.resolve('./react-js-blog-website-yt-3a79c-firebase-adminsdk-fbsvc-9d30863de3.json'), 'utf-8');
    ...
    process.exit(1); // Exit the process if critical error
  }
}
...
const generateUploadURL = async () => {
  ...
  return await s3.getSignedUrlPromise('putObject', {
    Bucket: 'blogging-website-yt-tutorialone',
    Key: imageName,
    Expires: 1000,
...
server.post("/add-comment", verifyJWT, (req, res) => {
  ...
  //creting a comment doc
  let commentObj = {
    blog_id: _id, blog_author, comment, commented_by: user_id, isReply: replying_to ? true : false, 
  }
...
server.post("/delete-blog", verifyJWT, async (req, res) => {
  try {
    ...
    // Only allow admins
    if (!isAdmin) {
      return res.status(403).json({ error: "you don't have prmissions to delete the blog" });
    }
    ...
    const deleteNotifications = await Notification.deleteMany({ blog: blog._id });
    console.log("Notifications deleted:", deleteNotifications.deletedCount);

    // Delete related comments
    const deleteComments = await Comment.deleteMany({ blog: blog._id });
    console.log("Comments deleted:", deleteComments.deletedCount);

    // Update the original blog author's profile
    const user = await User.findOneAndUpdate(
      { _id: blog.author }, // original author, not admin
      {
        $pull: { blog: blog._id },
        $inc: { "account_info.total_posts": -1 }
      }
    );
    console.log("Author updated:", user?.email || user?._id);
